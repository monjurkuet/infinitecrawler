...
| Repository root | `/run/media/growloop/codebase/infinitecrawler` | All commands relative to this. (Previously `/root/codebase/vhd/infinitecrawler` on the old deploy host — that path is stale; the working tree now lives here.) |
| Python env | `uv`-managed venv at `/home/growloop/.venvs/ic` | Created via `uv sync` (project `UV_PROJECT_ENVIRONMENT`). `uv run` works project-wide without manual export. `.venv/` in the repo is dead/root-owned — do NOT use it. |
| Listing daemon URL form (CRITICAL) | Must navigate via `?cid=<decimal>` not `/place/<name>/data=...` | Headless Chrome (pinchtab) cannot render the `/place/<name>/data=!...:0xAAAA:0xBBBB` detail view — Google strips the CID payload and bounces to a bare map shell, so every selector misses (only `name` or nothing extracted, "no_meaningful_data" retries). `daemons/listing_daemon.py:to_cid_url()` rewrites queued URLs to `https://www.google.com/maps/place/?cid=<decimal>` (CID = trailing `:0xHEX` in the `data=` blob). With `?cid=`, the full panel renders and name/rating/phone/address/plus_code all populate. Verified 2026-08-25: real listings now extract complete records (e.g. "Hotel Banani Inn" 5.0★, phone, address). |
| Pinchtab launch (this host) | `pinchtab bridge --port 9868` (npm binary at `~/.npm-global/bin/pinchtab`) | Run as a background process, NOT a systemd unit, on this machine. Requires `security.allowEvaluate=true` + `security.allowedDomains` incl. `google.com`/`www.google.com`/`maps.google.com`, and `server.token` matching `.env` `PINCHTAB_TOKEN`. |
...
| Pinchtab concurrent nav race (2026-08-12, CRITICAL) | `PinchtabClient.navigate()` used `/navigate` which spawns a new tab every call AND overwrites `self.tab`. With the listing daemon's tab-pool (`LISTING_TAB_POOL=3`) workers calling navigate concurrently, only the last call wins; earlier callers' returned `PinchtabTab` objects pointed to tabs that were never navigated (some even returned `about:blank` URLs). Effect: every extraction returned `None` for all fields, "no_meaningful_data" retries, 0 DB writes, daemon logged `⚠ All 2 retries failed for 'phone': None` etc. for every URL. Fixed by (1) adding `asyncio.Lock` on `PinchtabClient.navigate` (serializes at the client level), and (2) using tab-scoped endpoint `/tabs/{id}/navigate` from the listing daemon so workers reuse their existing tabs instead of allocating new ones (keeps total tab count bounded under pinchtab `maxTabs: 8` and avoids the "tab context dropped" orphan). Verified: listings throughput ~108/hr, flushes landing within ~25s of extraction. | `base/pinchtab_client.py:478-509`, `daemons/listing_daemon.py:367`. Symptom check: `grep -c 'All 2 retries failed for' infinitecrawler-listing.log` rising fast with `listings_1h=0` = concurrent-nav race. |
| Email-extract-loop idle-in-tx (2026-08-12) | `infinitecrawler-email-extract-loop.service` runs `--loop --loop-gap 30`, so each cycle the psycopg connection is open for the query + 30s sleep. The T3 PG session hardening default (`PG_IDLE_TX_TIMEOUT=30s`) fires on every cycle → `terminating connection due to idle-in-transaction timeout`, then the script reconnects. Drop-in override sets `Environment=PG_IDLE_TX_TIMEOUT=120s` (still safe vs runaway queries — `PG_STATEMENT_TIMEOUT=120s` is the real bound). | `~/.config/systemd/user/infinitecrawler-email-extract-loop.service.d/override.conf`. Detection: `grep 'idle-in-transaction timeout' infinitecrawler-email-extract-loop.log`. |
| GMaps `data-item-id="phone:tel:N…"` selector (2026-08-12) | Google Maps now renders phone buttons as `button[data-item-id="phone:tel:01718XXXXXX"]` (colon-prefixed, value is the E.164 number). The pre-existing `button[data-item-id^="phone"]` selector still matches via the prefix-attribute operator, but `a[data-item-id="authority"]` no longer matches for the website — present listings without a website still get `Add website` (not a real `authority` link). `h1.DUwDvf` works for name; the legacy `.qBF1Pd.fontHeadlineSmall` is stale. No new fallback needed for phone/address (existing `^=` selectors still resolve). | `config/gmaps_listings_working.yaml:47-119`. Test directly: `tab.select('button[data-item-id^="phone"]').text` → `'01718-652378'`. |
| Listing queue pollution from test items (2026-08-12) | `gmaps:pending` had 354 `testNNN` items (no `https://` prefix) plus `stale1`/`stale2`/`stale3` injected by an earlier session. Daemon processed them as URLs → `navigate("test218")` → pinchtab error → `mark_failed` → 3 retries → land in `gmaps:failed`. Drains the daemon and pollutes metrics. Cleanup: `redis-cli --raw LRANGE gmaps:pending 0 -1 \| grep '^test' \| while read u; do redis-cli LREM gmaps:pending 1 "$u"; done` + same for `^stale` and `HDEL` on failed. Then restart daemon to clear `processing` residue. | Manual one-shot; no code path emits test URLs today. |
| Pinchtab `maxTabs` cliff (2026-08-12) | Default `instanceDefaults.maxTabs: 8`. With listing daemon pool=3, search daemon's single tab, plus the `always-on` supervisor, hitting >8 causes pinchtab to evict tabs → workers see `tab context dropped (tab XXX not found), re-navigating to about:blank`. The tab-scoped `/tabs/{id}/navigate` fix (above) keeps each worker's tab stable; do NOT lower `maxTabs` without also lowering `LISTING_TAB_POOL`. | `/root/.pinchtab/config.json`. Live: tabs=6 after fix (was 12 before). |
| Per-tab-navigate endpoint discovery (2026-08-12) | Pinchtab 0.15 exposes `/tabs/{id}/navigate` (POST) in addition to `/navigate`. The tab-scoped variant reuses the existing tab context (returned `tabId` matches input); the unscoped endpoint allocates a new tab every call. Use the tab-scoped one whenever a `tab_id` is already known (listing daemon workers, search daemon single-tab path). | `base/pinchtab_client.py:navigate(tab_id=…)`. Verified via `curl /openapi.json` paths. |
...
| Listing daemon headless URL form (2026-08-25, CRITICAL) | Queued `/place/<name>/data=!...:0xAAAA:0xBBBB` URLs render ONLY a bare map shell in headless Chrome (pinchtab) — Google strips the CID payload, so the detail panel never loads and every selector misses ("All N retries failed for 'phone': None", "no_meaningful_data"). Fix: `to_cid_url()` in `daemons/listing_daemon.py` rewrites each URL to `https://www.google.com/maps/place/?cid=<decimal>` (decimal = trailing `:0xHEX` in the `data=` blob). With `?cid=`, the full panel renders and name/rating/phone/address/plus_code populate. Symptom check: if `gmaps_listings` rows have `phone IS NULL` for recently crawled URLs, the daemon is navigating `/place/data=` instead of `?cid=`. | `daemons/listing_daemon.py:to_cid_url()` (called at the navigate call in `process_url`). Verified live 2026-08-25: 28,740+ rows now have both phone AND rating; e.g. "Hotel Banani Inn" → 5.0★, 1 review, phone 01741070930, "R96J+JFP, বগুড়া". |
...
- Listing daemon: `⚠ All 2 retries failed for 'phone': None` (and rating/review_count/website/address) repeating hundreds of times, `listings_1h=0` despite "active" daemon → **pinchtab concurrent-nav race** (2026-08-12). Daemon is alive but every extraction returns None because workers raced on the same `PinchtabClient.tab` slot. Fix landed: `PinchtabClient.navigate` takes a client-level `asyncio.Lock`; listing daemon uses `/tabs/{id}/navigate` to reuse tabs.
- Listing daemon logs `tab context dropped (tab XXX not found), re-navigating to about:blank` repeatedly → pinchtab evicted the worker's tab because total tabs > `maxTabs: 8`. Same root cause as above; fix holds the pool at ~3-6 tabs.
- Email-extract-loop logs `terminating connection due to idle-in-transaction timeout` every cycle → `PG_IDLE_TX_TIMEOUT` (30s default) shorter than `--loop-gap 30` + query. Raise via drop-in `Environment=PG_IDLE_TX_TIMEOUT=120s`.
- `gmaps:pending` contains `testNNN` / `staleN` strings (no `https://` prefix) → leftover from prior test run. Drain via `LREM` + `HDEL` on `gmaps:failed`.
- `linkedin-search`/`classify`/`linkedin-match` count via `pgrep` > the systemd-managed count (or `NRestarts` climbs without restart storms) → orphan nohup-spawned processes accumulated. Kill orphans with `pkill -KILL -f 'db_linkedin_search|db_classify|match_linkedin_to_gmaps'` and verify `pgrep -af` returns nothing. If recurrence after watchdog run → regression in launch path (post-2026-08-13 feadcfc this should not happen).
...
# 6j. Pinchtab concurrent-nav race repair (2026-08-12)
# Symptom: `grep -c 'All 2 retries failed for' infinitecrawler-listing.log` grows
# by ~hundreds per minute AND `listings_1h == 0`. The listing daemon is "active"
# but every extraction returns None because PinchtabClient.tab got overwritten
# by a sibling worker before the page loaded.
# Verify the fix is in place first:
grep -n "_navigate_lock\|tab_id=tab._tab_id" base/pinchtab_client.py daemons/listing_daemon.py
# Then restart listing daemon and the tab pool rebuilds under the lock:
systemctl --user restart infinitecrawler-listing
# Confirm within 2-3 min: `listings_5m > 0` in monitor output AND `grep 'Extracted 1 fields from' infinitecrawler-listing.log` shows real runs.

# 6k. Drain test/stale URLs from listing queue (one-shot)
redis-cli --raw LRANGE gmaps:pending 0 -1 | grep -E "^(test|stale)" | while read u; do redis-cli LREM gmaps:pending 1 "$u" >/dev/null; done
redis-cli --raw HKEYS gmaps:failed  | grep -E "^(test|stale)" | while read u; do redis-cli HDEL gmaps:failed "$u" >/dev/null; done
# (Run on the listing daemon's Redis namespace — adjust if a different namespace is in use.)

# 6l. Kill orphaned daemon processes (legacy nohup leftovers, 2026-08-13 feadcfc)
# Use SIGKILL — nohup'd processes ignore SIGTERM (that's the point of nohup).
pkill -KILL -f 'db_linkedin_search\.py' 2>/dev/null || true
pkill -KILL -f 'db_classify\.py'       2>/dev/null || true
pkill -KILL -f 'match_linkedin_to_gmaps\.py' 2>/dev/null || true
# DO NOT kill firehose/listing/search/email_extract — those are systemd-managed.
# After cleanup, restart the 4 perpetual daemons via systemctl so watchdog's
# next invocation finds them via `pgrep` and skips re-spawning:
for u in infinitecrawler-listing infinitecrawler-search \
         infinitecrawler-email-extract-loop infinitecrawler-linkedin-firehose-loop; do
  systemctl --user restart "$u.service" >/dev/null 2>&1 || true
done
systemctl --user reset-failed infinitecrawler-watchdog.service
# Verify: pgrep -af 'db_linkedin_search|db_classify|match_linkedin_to_gmaps' returns nothing.
```
