    ...
    for brand in known_brands:
        if brand.lower() in cleaned.lower():
            ...
            while start > 0 and cleaned[start-1] not in (' ','-','.','|'):
                if cleaned[max(0,start-2):start].lower() == 'at': break
                start -= 1
            end = idx + len(brand)
            while end < len(cleaned) and cleaned[end] not in (' ','-','.','|'):
                chunk = cleaned[end:end+6].lower()
                if any(chunk.startswith(x) for x in ('marke','compa','corpo','indust')): break
                end += 1
...
def update_companies(conn, dry_run):
    ...
    if dry_run:
        for tid, title in luxury[:30]:
            ...
            if c: log.info("  [lux id=%d] '%s' -> '%s'", tid, (title or '')[:45], c)
        if len(luxury) > 30: log.info("  ... %d more luxury", len(luxury) - 30)
        for tid, title in discovered[:15]:
            c = extract_company(title)
            if c: log.info("  [disc id=%d] '%s' -> '%s'", tid, (title or '')[:45], c)
        return 0
    updated = 0
